...
async def app(scope, receive, send):
    ...
    try:
        ...
        await send(
            {
                "type": "http.response.body",
                "body": json.dumps({"error": "Failed to fetch currency data"}).encode(),
            }
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run("currency_proxy:app", host="127.0.0.1", port=8000, reload=True)
